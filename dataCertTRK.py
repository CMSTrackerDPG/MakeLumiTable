...
class Certifier():
    ...
    def generateFilter(self):
        ...
        for qf in self.qflist:
            ...
            if sys != "NONE":
                # Check if the bit is not excluded to avoid filter on LS for Egamma, Muon, JetMET
        ...
        if len(self.beamene):
            ...
            if (len(self.qflist) != 0):
                  if (self.qflist[0] == 'NONE:NONE'):
                    queryStr = eneQuery
                    for id_,dcs in enumerate(self.dcslist):
                       if (id_ == 0):
                            queryStr +=  ' AND ({' + dcs.lower() + "Ready} = 1"
                       else:
                            queryStr += ' OR {' + dcs.lower() + "Ready} = 1"

                    queryStr += " OR {bpixReady} = 1 OR {fpixReady} = 1) AND ({beam1Stable} = 1 AND {beam2Stable} = 1)"
                    self.filter.setdefault("dataset", {})\
                                                      .setdefault("filter", {})\
                                                      .setdefault("run", {})\
                                                          .setdefault("query", queryStr)
			...
			queryStr += " AND {bpixReady} = 1 AND {fpixReady} = 1) AND ({beam1Stable} = 1 AND {beam2Stable} = 1)"
			self.filter.setdefault("dataset", {})\
							  .setdefault("filter", {})\
							  .setdefault("run", {})\
							  .setdefault("query", queryStr)
    ...
    def generateJson(self):
        try:
            ...
            print e
            sys.exit(1)
# Modification needed to use TRACKER workspace in RR
#        self.cert_json = self.api.data(workspace = 'GLOBAL'\
        self.cert_json = self.api.data(workspace = 'TRACKER'\
                                       , table = 'datasetlumis'\
                                       , template = 'json'\
...
def get_dasjson(self, datasets, runmin, runmax, runlist):
    ...
    for ds in  datasets.split(","):
        for runnm in range(int(runmin), int(runmax)+1):
            foundrun = False
            if len(runlist)>0:
                foundrun = runNum in self.runlist
            else:
                foundrun = True
